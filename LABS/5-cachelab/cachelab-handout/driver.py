...
def main():
    ...
    print("Running ./test-trans -q -M 32 -N 32")
    p = subprocess.Popen("./test-trans -q -M 32 -N 32 | grep TEST_TRANS_RESULTS",
                         shell=True, stdout=subprocess.PIPE)
    stdout_data = p.communicate()[0].decode('utf-8')
    result32 = re.findall(r'(\d+)', stdout_data)
    
    # 64x64 transpose
    print("Running ./test-trans -q -M 64 -N 64")
    p = subprocess.Popen("./test-trans -q -M 64 -N 64 | grep TEST_TRANS_RESULTS",
                         shell=True, stdout=subprocess.PIPE)
    stdout_data = p.communicate()[0].decode('utf-8')
    result64 = re.findall(r'(\d+)', stdout_data)
    
    # 60x68 transpose
    print("Running ./test-trans -q -M 60 -N 68")
    p = subprocess.Popen("./test-trans -q -M 60 -N 68 | grep TEST_TRANS_RESULTS",
                         shell=True, stdout=subprocess.PIPE)
    stdout_data = p.communicate()[0].decode('utf-8')
