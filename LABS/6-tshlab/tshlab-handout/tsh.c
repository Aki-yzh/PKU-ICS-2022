...
//因为经常用到，大段显得繁琐，故创建个waitfg函数等待fg job的结束
...
eval(char *cmdline) 
{
    ...
    if (tok.argv[0] == NULL) /* ignore empty lines */
        return;
    //由writeup得知总是先假设name是可执行文件路径
    if(tok.infile!=NULL)
    {   
        int fd = open(tok.infile,O_RDONLY);
        if(fd<0)
        {
            sio_puts("Unable to open trace file ");
            sio_puts(tok.infile);
            sio_puts("\n");//测试tshref得知的输出语句
            return;
        }
    ...
    if(tok.outfile!=NULL)
    {
        int fd=open(tok.outfile,O_RDWR|O_CREAT);
        if(fd<0)
        {
            sio_puts("Unable to open trace file ");
            sio_puts(tok.outfile);
            sio_puts("\n");//测试tshref得知的输出语句
            return;
        }
        fdout = dup(1);
    ...
    //先处理builtin_cmd,利用switch转向不同的处理
    ...
    if(tok.builtins)
    {
        ...
        {   
            ...
            case BUILTIN_KILL:
            {   
                char *s = tok.argv[1];//经常要用，设个变量更方便
                struct job_t* job;
                if(tok.argv[1]==NULL)
                {
                    printf("%s ",tok.argv[0]);
    ...
    else//run program
    {
        ...
        // mask_all屏蔽所有，mask_three屏蔽三种信号，mask_prev记录原先的屏蔽情况
        sigfillset(&mask_all);
...
{
    int old_errno = errno;//存储旧的errno
    sigset_t mask_all,mask_prev;//含义同上方
    pid_t pid;
    int status;
    sigfillset(&mask_all);
    ...
    {
        sigprocmask(SIG_BLOCK,&mask_all,&mask_prev);
        if(WIFEXITED(status))//运行结束
        {
            deletejob(job_list,pid);
        }
        else if(WIFSIGNALED(status))//终止
        {   
            int t=WTERMSIG(status);//用于减少长度
            int p=pid2jid(pid);
        ...
        else if(WIFSTOPPED(status))//停止
        {
            ...
            int t=WSTOPSIG(status);
            int p=pid2jid(pid);//用于减少长度
 ...
 */
 //捕获SIGINT信号，传递给前台作业中的每个进程。默认操作是终止进程
void 
sigint_handler(int sig) 
{   
    int old_errno =errno;
    sigset_t mask_all,mask_prev;//含义同上
    pid_t pid= fgpid(job_list);
    sigfillset(&mask_all);
    sigprocmask(SIG_BLOCK,&mask_all,&mask_prev);  
    if(pid)//有foreground job
    {
        kill(-pid,SIGINT);//writeup中提示的kill-pid
    }
    sigprocmask(SIG_SETMASK,&mask_prev,NULL);
    errno = old_errno;
    return;
 ...
 */
 //捕获SIGTSTP信号，传递给前台作业中的每个进程，将进程置于停止状态
void 
...
{   
    ...
    if(pid)//有foreground job
    {
        kill(-pid,SIGTSTP);//writeup中提示的kill-pid
    }
    sigprocmask(SIG_SETMASK,&mask_prev,NULL);
    errno = old_errno;
    return;
